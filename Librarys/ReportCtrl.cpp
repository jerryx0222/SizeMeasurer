...
{	
	...
	//std::vector<std::map<int,bool>* > m_ItemEditable;
	for (int i = 0; i < m_ItemEditable.size(); i++)
		delete m_ItemEditable[i];
	m_ItemEditable.clear();
...
{
	if (m_WndEdit.m_hWnd != NULL && m_WndEdit.IsWindowVisible())
...
void CReportCtrl::OnCustomDraw(NMHDR* pNMHDR, LRESULT* pResult) 
{
    ...
    else if (lplvcd->nmcd.dwDrawStage == (CDDS_ITEMPREPAINT | CDDS_SUBITEM))
    {
		...
		/*
		dwValue = lplvcd->nmcd.dwItemSpec;
		pItem = CListCtrl::GetItemData(dwValue);
		CItemData* p = (CItemData*)pItem;
		ASSERT(p != NULL);
...
void CReportCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (m_hParent != NULL)
		::PostMessage(m_hParent, WM_LISTBTNDOWN, (WPARAM)this, NULL);
}
...
void CReportCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	...
	if(m_hParent!=NULL)
		::PostMessage(m_hParent,WM_LISTDBLCLK,(WPARAM)this,NULL);
}
...
void CReportCtrl::_MouseClkMonitor(UINT nMsg, UINT nFlags, CPoint point, BOOL bTriggerEdit)
{
	...
	//std::vector<std::map<int, BOOL>* > m_ItemEditable;
	std::map<int, BOOL>* pDatas;
	std::map<int, BOOL>::iterator itMap;
	...
	{
		if (m_ItemEditable.size()<=0)
			StartEdit(IDX, hti.iSubItem);
		else
		{
			if (m_ItemEditable.size() > hti.iItem)
			{
				pDatas = m_ItemEditable[hti.iItem];
				itMap = pDatas->find(hti.iSubItem);
				if (itMap != pDatas->end())
				{
					if (itMap->second) StartEdit(IDX, hti.iSubItem);
				}
			}
		}
	}
...
BOOL CReportCtrl::SetItemEdit(int nItem, int nSubItem, BOOL bEnable)
{
	std::map<int, BOOL>::iterator itMap;
	std::map<int, BOOL>*	pDatas;
	/*if (nItem < m_ItemEditable.size()) 
		return FALSE;
	else */if (nItem == m_ItemEditable.size())
	{
		pDatas = new std::map<int, BOOL>;
		m_ItemEditable.push_back(pDatas);
	}
	else if (nItem > m_ItemEditable.size())
	{
		return false;
	...
	{
		pDatas = m_ItemEditable[nItem];
	}
	itMap = pDatas->find(nSubItem);
	if (itMap != pDatas->end())
	{
		itMap->second = bEnable;
	}
	else
	{
		pDatas->insert(std::make_pair(nSubItem, bEnable));
	}
	return true;	
}
...
BOOL CReportCtrl::StartEdit(int nItem, int nSubItem)
{
	...
	if (FMT != m_dwPrevEditFmt)
	{
		...
		if (m_WndEdit.GetSafeHwnd() != NULL) m_WndEdit.DestroyWindow();
		if (!m_WndEdit.Create(ES_AUTOHSCROLL | ES_NOHIDESEL | WS_CHILD | WS_BORDER | FMT, CRect(0, 0, 1, 1), this, 0))
			return FALSE;
	}
	else
	{
		if (m_WndEdit.GetSafeHwnd() == NULL
			&& !m_WndEdit.Create(ES_AUTOHSCROLL | ES_NOHIDESEL | WS_CHILD | WS_BORDER | FMT, CRect(0, 0, 1, 1), this, 0))
		{
			return FALSE;
		}
	}
	...
	strShow = GetItemText(m_ptEditting.x, m_ptEditting.y);
	m_WndEdit.SetWindowText(strShow);
...
BOOL CReportCtrl::EndEdit(BOOL bCommit)
{
	...
	if (bChanged)
	{
		...
		hti.pt = m_ptEditting;
		IDX = SubItemHitTest(&hti);
		//GetParent()->SendMessage(WM_REPORTITEMEDIT, (WPARAM)hti.iItem, (LPARAM)hti.iSubItem);
		GetParent()->SendMessage(WM_REPORTITEMEDIT, (WPARAM)m_ptEditting.x, (LPARAM)m_ptEditting.y);
...
void CReportCtrl::SetFontSize(int pixelHeight)
{
