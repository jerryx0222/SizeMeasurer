...
bool HCamera::Open()
{
    ...
    if(m_AcqHandle!=0)
        Close();
    m_ImageFiles.clear();
    if((strParam.indexOf("C:/")>=0) || (strParam.indexOf("D:/")>=0))
    {
        SetImageFilesParameters(strParam);
        return m_ImageFiles.size()>0;
    }
    ...
    try{
        ...
        HalconCpp::OpenFramegrabber("GigEVision2",
                                    0, 0, 0, 0, 0, 0,
                                    field,
                                    m_nImageDepth,
                                    "default",
        ...
        if(m_AcqHandle!=0)
        {
            ...
            else if(strParam.indexOf("Basler"))
                SetBaslerParameters();
            else
            {
                return false;
...
{
    QString strParam=QString::fromStdWString(m_strParameter.c_str());
    if(m_AcqHandle==0) return nullptr;
    try{
        HalconCpp::HImage *pHImage=new HalconCpp::HImage();
        //HalconCpp::GrabImage(&image,m_AcqHandle);
       // HalconCpp::GrabImageStart(m_AcqHandle,-1);
        ...
        //HalconCpp::CloseFramegrabber(m_AcqHandle);
        //QImage *pImgOut=HHalconLibrary::Hobject2QImage(image);
        return pHImage;
    }catch(HalconCpp::HException&)
    {
    }
...
void HCamera::SetImgSourceParameters()
{
    ...
    QString strParam;
     try{
        // ImageSource
        HalconCpp::SetFramegrabberParam(m_AcqHandle, "ExposureAuto", "Off");
        HalconCpp::SetFramegrabberParam(m_AcqHandle, "ExposureTime", 2000);

        //HalconCpp::SetFramegrabberParam(m_AcqHandle, "ExposureAutoReference", "10");  // 0~255

        HalconCpp::SetFramegrabberParam(m_AcqHandle, "GainAuto",    "Off");
        HalconCpp::SetFramegrabberParam(m_AcqHandle, "Gain",        8);      // 0~27dB
...
void HCamera::SetImageFilesParameters(QString strPath)
{
    //m_ImageFiles
