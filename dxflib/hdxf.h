...
struct DLPolylinesData:public DL_PolylineData
{
public:
    DLPolylinesData(int pNumber, int pMVerteces, int pNVerteces, int pFlags)
        :DL_PolylineData(pNumber,pMVerteces,pNVerteces,pFlags)
    {
    }

    std::vector<DL_VertexData> vDatas;
};


class HDxf : public DL_CreationInterface
{
   // Q_OBJECT
public:
    HDxf();
    ...
    bool ReadDxfFile(std::string strFile,double unit);
    ...
    void CopyLineData(QPointF& center,std::vector<QLineF*>& vLines);
    void CopyLineData(QPointF& center,std::vector<QLineF*>& vLines,QRectF& rect);
    void CopyPLineData(std::vector<std::vector<QPointF*>*>& vPLines,QRectF& rect);
    void CopyPLineData(int id,double dblMinPitch,std::vector<QPointF*>& vPLine,QRectF& rect);
    void GetRect(QRectF &rerct);

    int GetPLinesDataCount()
    {
        return static_cast<int>(m_mapPLDatas.size());
    }

private:
    void ClearMapLines();
    void TransArc2Points(QPointF *pptS,QPointF ptT,double bulge,double pitch,std::vector<QPointF*> &vPoints);
    void TransLine2Points(QPointF *pptS,QPointF ptT,double pitch,std::vector<QPointF*> &vPoints);

private:
    std::string     m_strACADVer;
    double          m_dblMax[3],m_dblMin[3],m_dblUnit;

    DL_Dxf m_dxf;


    //DL_LayerData    *m_pLayerData;
    DL_CircleData   *m_pCircleBase;

    std::map<int,DL_LineData*> m_mapLineDatas;
    std::map<int,DLPolylinesData*> m_mapPLDatas;


    std::map<int,DL_BlockData*> m_mapBlockDatas;
    DL_BlockData* m_pOptBlockData;
};
};
#endif // HDXF_H
