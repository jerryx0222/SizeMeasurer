...
bool DL_Dxf::in(const std::wstring& file, DL_CreationInterface* creationInterface) {
    ...
    currentEntity = DL_UNKNOWN;
    int errorCounter = 0;
	std::wstring	strT=converToWideChar("rt");

	fp = _wfopen(file.c_str(),strT.c_str());
    if (fp) {
        while (readDxfGroups(fp, creationInterface, &errorCounter)) {}
        fclose(fp);
        if (errorCounter>0) {
...
bool DL_Dxf::processDXFGroup(DL_CreationInterface* creationInterface,
    ...
    // Indicates start of new entity or var
        ...
        if ((values[8])[0]!='\0') {
            //strcpy_s(name,DL_DXF_MAXLINE+1, values[8]);
...
void DL_Dxf::writeMText(DL_WriterA& dw,
    ...
    unsigned int length = (unsigned int)data.text.length();
    char chunk[251];
    unsigned int i;
